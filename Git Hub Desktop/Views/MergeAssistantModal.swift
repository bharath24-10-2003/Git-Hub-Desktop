...
struct MergeAssistantModal: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            // Header Banner: Merge in Progress
        ...
        .onAppear {
            editedMessage = viewModel.mergeState.defaultCommitMessage
            selectedFile = nil
            viewModel.selectedFileForDiff = nil
            viewModel.currentDiff = nil
        }
        .onChange(of: viewModel.mergeState.defaultCommitMessage) { _, newValue in
    ...
    private var mergeStatusCard: some View {
        ...
        VStack(alignment: .leading, spacing: 6) {
            ...
            TextField("Commit Message", text: $editedMessage)
                ...
                .onChange(of: editedMessage) { _, newValue in
                    viewModel.setMergeMessage(newValue, at: repo)
                }
        }
        .padding(10)
    ...
    private var footerToolbar: some View {
        HStack {
            // Destructive: Abort Merge
            Button("Abort Merge") {
                Task {
                    await viewModel.abortMerge(at: repo)
                }
            }
            .buttonStyle(.bordered)
            .tint(.red)
            ...
            // Primary: Continue Merge
            Button("Continue Merge") {
                Task {
                    await viewModel.continueMerge(at: repo)
                }
            }
